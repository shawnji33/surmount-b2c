...
.sidebar-nav-btn{width:32px;height:32px;border-radius:var(--radius-md,8px);border:none;background:transparent;display:flex;align-items:center;justify-content:center;cursor:pointer;color:var(--color-gray-lm-500,#717680);transition:background 120ms ease,color 120ms ease;}
.sidebar-nav-btn svg{width:18px;height:18px;fill:none;flex-shrink:0;stroke:currentColor;}
.sidebar-nav-btn:hover{background:var(--color-gray-lm-200,#e9e9eb);color:var(--color-gray-lm-900,#181d27);}
.sidebar-nav-btn.active{background:var(--color-gray-lm-200,#e9e9eb);color:var(--color-gray-lm-900,#181d27);}
.sidebar-tooltip{position:absolute;left:calc(100% + 8px);top:50%;transform:translateY(-50%) translateX(-6px);background:var(--color-gray-lm-900,#181d27);color:#fff;font-size:11px;font-weight:500;font-family:var(--font-family-body,'Geist',sans-serif);padding:5px 8px;border-radius:var(--radius-sm,6px);white-space:nowrap;opacity:0;pointer-events:none;transition:opacity 150ms ease,transform 150ms ease;z-index:400;box-shadow:0 2px 8px rgba(10,13,18,0.18);}
.sidebar-tooltip::before{content:'';position:absolute;right:100%;top:50%;transform:translateY(-50%);border:5px solid transparent;border-right-color:var(--color-gray-lm-900,#181d27);}
.sidebar-nav-item:hover .sidebar-tooltip{opacity:1;transform:translateY(-50%) translateX(0);}
.sidebar-avatar{width:32px;height:32px;border-radius:50%;background:var(--color-bg-secondary,#fafafa);border:0.75px solid rgba(0,0,0,0.08);box-shadow:var(--shadow-card,0 2px 12px rgba(10,13,18,0.03));display:flex;align-items:center;justify-content:center;cursor:pointer;overflow:hidden;flex-shrink:0;transition:box-shadow 150ms ease;}
.sidebar-avatar:hover{box-shadow:0 0 0 3px rgba(123,120,214,0.2);}
.sidebar-avatar-inner{width:18px;height:18px;border-radius:50%;background:#7b78d6;border:0.5px solid rgba(0,0,0,0.06);display:flex;align-items:center;justify-content:center;font-size:10px;font-weight:500;color:#fff;font-family:var(--font-family-body,'Geist',sans-serif);letter-spacing:0;}
</style>`;
...
const NAV_ITEMS = [
  ...
  },
  {
    key: 'marketplace',
    label: 'Marketplace',
    href: '../marketplace/marketplace.html',
...
function sidebarNavigate(href) {
  // Always do a full page navigation — shell pages each have their own iframe setup.
