...
function aggregateSeries(accounts: InvestingAccount[]) {
  return DATES.map((time, index) => ({
    time,
    value: Number(sumValues(accounts.map((account) => account.data[index]?.value ?? 0)).toFixed(2)),
  }));
}
...
function PortfolioAccountSwitcher({
  ...
  const selectedAccounts = INVESTING_ACCOUNTS.filter((account) => selected.has(account.id));
  ...
  // FLIP width animation — fires after DOM updates with new label
    ...
    <>
      ...
      >
        <span className={s.switcherAvatarStack} aria-hidden="true">
          {allSelected && (
            <span className={s.switcherAllIcon}>
              <SquaresFour weight="bold" />
            </span>
          )}
          {!allSelected && visibleAccounts.map((account) => (
            <AccountLogo key={account.id} account={account} className={s.switcherAvatar} />
          ))}
...
export default function AccountSelectionPlaygroundPage() {
  const [selected, setSelected] = useState<Set<string>>(
    () => new Set(INVESTING_ACCOUNTS.map((account) => account.id)),
  );

  const selectedAccounts = useMemo(
    () => INVESTING_ACCOUNTS.filter((account) => selected.has(account.id)),
    [selected],
  );

  const chartData = useMemo(() => aggregateSeries(selectedAccounts), [selectedAccounts]);
  const totalValue = useMemo(
    () => sumValues(selectedAccounts.map((account) => account.value)),
    [selectedAccounts],
  );
  const totalChange = useMemo(
    () => sumValues(selectedAccounts.map((account) => account.change)),
    [selectedAccounts],
  );
  ...
  const visibleStrategies = useMemo(() => (
    STRATEGIES.map((strategy) => {
      const accountIds = Object.keys(strategy.accounts).filter((accountId) => selected.has(accountId));
      const value = sumValues(accountIds.map((accountId) => strategy.accounts[accountId].value));
      const returnAbs = sumValues(accountIds.map((accountId) => strategy.accounts[accountId].returnAbs));
      const costBasis = value - returnAbs;
      const returnPct = costBasis !== 0 ? (returnAbs / costBasis) * 100 : 0;
      const accounts = INVESTING_ACCOUNTS.filter((account) => accountIds.includes(account.id));
    ...
    }).filter((strategy) => strategy.accounts.length > 0)
  ), [selected]);
  ...
  return (
    <main className={s.main}>
      <DashboardHeader title="Account selection playground" />
          ...
          <div className={s.savingAccounts}>
            ...
            <div className={s.savingPromptCard}>
              ...
              <div className={s.savingPromptActions}>
                ...
                <button type="button" className={s.dismissSavingPrompt} aria-label="Dismiss high yield cash account prompt">
