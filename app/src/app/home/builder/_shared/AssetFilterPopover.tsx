...
import type { AssetFilterState, MarketCapBucket } from './types';
import { countActiveFilters } from './_lib/assetFilters';
import s from './AssetFilterPopover.module.css';

const INDUSTRIES = Array.from(new Set(ASSET_UNIVERSE.map((a) => a.industry))).sort();
...
// Industries aren't mutually exclusive the way price/market-cap brackets are — a user comparing
// "Semiconductors" and "Cloud Software" names wants both checked at once, not a single pick — so
// this is checkboxes (multi-select), not a ChipGroup (single-select).
...
export function AssetFilterPopover({
  ...
  /** Shrinks the trigger to match a 32px-tall compact search bar (InlineAddAsset) instead of the
   * default 44px that matches PickStocksStep's full-size ClearableSearchInput. The 44px touch
   * target is preserved either way, via a pseudo-element hit-area on the compact variant. */
  compact?: boolean;
}) {
  ...
  const panel = (
    <div
      ...
      className={s.panel}
      data-open={open}
      data-portal-panel="asset-filter"
      inert={!open}
    ...
    >
      <div className={s.scrollArea}>
        ...
        <IndustryCheckboxGroup options={INDUSTRIES} value={filters.industries} onToggle={toggleIndustry} />
      </div>
