...
export function Carousel({ variant = 'default' }: { variant?: 'default' | 'empty' }) {
  ...
  const pickStrategySlide = (
    <a key="strategy" className={[s.carouselSlide, s.strategyCarouselSlide].join(' ')} href="/onboarding/pick-strategy">
  ...
  // Empty state: Surmount-account poster first, then the savings promo — no
  // "pick your first strategy" (that belongs to the investing-account flow).
  const slides = empty
    ? [surmountSlide, boostSlide]
    : [pickStrategySlide, boostSlide, surmountSlide];
